...
class ProductSupplierInfo(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        use_customer_code = bool(
            int(self.env["ir.config_parameter"].sudo().get_param("product_supplierinfo_for_customer"
                                                                 ".use_customer_code", 1)))
        res = super(ProductSupplierInfo, self).search(
            args, offset=offset, limit=limit, order=order, count=count)
        if self.env.context.get('customerinfo') and use_customer_code and \
